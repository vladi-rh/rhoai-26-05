...
. Create a new directory named `data` in the root folder and upload the following files (test.csv, train.csv,validate.csv) +
You can download these files by clicking the link below: 
+
* https://github.com/rh-aiservices-bu/fraud-detection/blob/main/data/test.csv[test.csv]
+
* https://github.com/rh-aiservices-bu/fraud-detection/blob/main/data/train.csv[train.csv]
+
* https://github.com/rh-aiservices-bu/fraud-detection/blob/main/data/validate.csv[validate.csv]

=== Create notebook to train a model

. Open your existing workbench in `userX-workshop` AI project. + 
Click on Launcher and create a new python notebook `1.train_model.ipynb`
+

[.bordershadow]
image::07_pipelines_elyra_notebook.png[]
...
.Did you face some issues 🐉 ? Here you can take a look at solution
[%collapsible]
====
Probably you got an exception stating that one or more variables are missing. +
You can either stop the workbench and add missing environment variables or add them directly in the notebook to move on faster.

[source,python]
----
region_name = 'wonderland'
bucket_name = 'models'
----
+ 
...
+
[.bordershadow]
image::07_pipelines_elyra_runtime.png[] 
